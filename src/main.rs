...
use geometry::{vector3::Vector3, point::Point3};
use material::{ DiffuseMaterial, DielectricMaterial, EmissiveMaterial };
...
fn main() -> std::io::Result<()> {
    ...
    // Framebuffer
    let mut framebuffer = vec![Vector3::default(); image_height * image_width];
    ...
    // let world_mat = WorldMaterial::new(Vector3::from_hex("#87CEEB"), Vector3::from_hex("#005d7d"));
    // let world_mat = WorldMaterial::new(Vector3::from_hex("#000"), Vector3::from_hex("#000"));
    // let world_sphere = Sphere::new(Vector3::new(0.0, 0.0, 0.0), 200.0, &world_mat);
    ...
    // Box
    let box_white_mat = DiffuseMaterial::new(Vector3::from_hex("#fff"));
    let box_green_mat = DiffuseMaterial::new(Vector3::from_hex("#0f0"));
    let box_red_mat = DiffuseMaterial::new(Vector3::from_hex("#f00"));
    let lightsource_mat = EmissiveMaterial::new(Vector3::from_hex("#fff"), 4.0);

    let xz_plane_points = vec![
        Vector3::new(-box_size/2.0, 0.0,  box_size/2.0),
        Vector3::new(-box_size/2.0, 0.0, -box_size/2.0),
        Vector3::new( box_size/2.0, 0.0, -box_size/2.0),
        Vector3::new( box_size/2.0, 0.0,  box_size/2.0),
    ];
    let yz_plane_points = vec![
        Vector3::new(0.0,  box_size/2.0, -box_size/2.0),
        Vector3::new(0.0,  box_size/2.0,  box_size/2.0),
        Vector3::new(0.0, -box_size/2.0,  box_size/2.0),
        Vector3::new(0.0, -box_size/2.0, -box_size/2.0),
    ];
    let xy_plane_points = vec![
        Vector3::new(-box_size/2.0,  box_size/2.0,  0.0),
        Vector3::new( box_size/2.0,  box_size/2.0,  0.0),
        Vector3::new( box_size/2.0, -box_size/2.0,  0.0),
        Vector3::new(-box_size/2.0, -box_size/2.0,  0.0),
    ];
    ...
    let box_top_mesh = Mesh::new(
        Point3::new(0.0, box_size/2.0, 0.0), 
    ...
    let box_back_mesh = Mesh::new(
        Point3::new(0.0, 0.0, box_size/2.0), 
        triangulate_square(&xy_plane_points), 
        &box_white_mat
    );
    let box_left_mesh = Mesh::new(
        Point3::new(-box_size/2.0, 0.0, 0.0), 
